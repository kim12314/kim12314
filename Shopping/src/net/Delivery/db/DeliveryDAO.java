...
public class DeliveryDAO {
   ...
   public List getListDelivery (int num) throws SQLException {
      ...
      try {
         ...
         while (re.next()) {
            ...
            deliverybean.setDelivery_product(re.getString("delivery_product"));
            deliverybean.setDelivery_memo(re.getString("delivery_memo"));
   ...
   //배달 주문 삭제
   public boolean deleteDelivery(int num) throws SQLException {
      String sql = "delete from delivery where delivery_num = ?";
      try {
         pt = conn.prepareStatement(sql);
         pt.setInt(1, num);
         pt.executeUpdate();
